...
abstract class MyTree extends ITreeBuilder with Iterable[Any]
{
  class Node
  {
    var pLeft: Node = null
  ...
  def iterator: Iterator[Any] = {
   ...
   class MyTreeIterator extends Iterator[Any]
  {
    var curr: Node = root
    var sizeIt: Int = 0
  ...
  def printAtGivenLevel(curNode: Node, level: Int): Unit = {
    if(curNode == null) print (" _ ")
    else if (level == 1) print(curNode.data + " ");
    else {
